...
| Hero video | `heroVideo` in `src/config/site.ts` — a filename in `public/hero/`, or `""` for the animated logo mark alone. It runs as a full-bleed backdrop behind the hero under a white veil; `heroVideoFocus` nudges which part stays in frame (CSS `object-position`). Loads only on ≥1024px with motion allowed; `?hero=1`…`?hero=4` compares clips on the live site. Clips are 1280×720, silent, ~2.05 MB each with `+faststart`. Re-encode with `ffmpeg -i in.mp4 -vf scale=1280:-2 -c:v libx264 -crf 30 -preset slow -movflags +faststart -an out.mp4` if you replace them. |
