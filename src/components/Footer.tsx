...
export default async function Footer({ locale }: { locale: string }) {
  ...
  return (
    <footer className="dark-s border-t border-line-dark">
      <div className="container-site pb-10 pt-16">
        <div className="grid grid-cols-2 gap-x-8 gap-y-10 md:grid-cols-[4fr_2fr_2fr_2fr]">
          <div className="col-span-2 md:col-span-1">
            <Image src="/brand/novieri-isotipo-blanco-256px.png" alt="" width={38} height={39} />
            <p className="mt-4 max-w-[34ch] text-[14.5px] text-on-dark-muted">{t("description")}</p>
          </div>
          <nav aria-label={t("servicesTitle")}>
            <h2 className="idx-mono mb-3.5 lowercase text-gold-bright">·· {t("servicesTitle")}</h2>
            <ul className="grid gap-2.5 text-[14.5px]">
              {SERVICE_LINKS.map((s) => (
                <li key={s.key}>
                  <Link href={s.href} className="text-on-dark-muted transition-colors hover:text-on-dark">
                    {tp(`${s.key}.name`)}
                  </Link>
                </li>
              ))}
          ...
          <nav aria-label={t("companyTitle")}>
            <h2 className="idx-mono mb-3.5 lowercase text-gold-bright">·· {t("companyTitle")}</h2>
            <ul className="grid gap-2.5 text-[14.5px]">
              <li>
                <Link href="/about" className="text-on-dark-muted transition-colors hover:text-on-dark">
                  {tn("about")}
                </Link>
              </li>
              <li>
                <Link href="/contact" className="text-on-dark-muted transition-colors hover:text-on-dark">
                  {tn("contact")}
                </Link>
              </li>
            </ul>
          ...
          <div>
            <h2 className="idx-mono mb-3.5 lowercase text-gold-bright">·· {t("contactTitle")}</h2>
            <ul className="grid gap-2.5 text-[14.5px] text-on-dark-muted">
              <li>
                <a href={`mailto:${site.contactEmail}`} className="transition-colors hover:text-on-dark">
                  {site.contactEmail}
                </a>
              </li>
              {site.whatsappNumber && (
        ...
        <div className="idx-mono mt-14 flex flex-wrap items-center justify-between gap-4 border-t border-line-dark pt-6 text-on-dark-faint">
          <span>
